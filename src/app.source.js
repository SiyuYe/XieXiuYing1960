...
const CACHE_CLEANUP_VERSION='xxy.cacheCleanup.v82110';
...
function cleanupLegacyCachesOnce_(){
 ...
 try{
  ...
  for(let i=0;i<localStorage.length;i++){
   ...
   if(key.indexOf('xxy.static.')===0||key.indexOf('xxy.siteBundle.')===0||key.indexOf('xxy.cms.v')===0||key.indexOf(SITE_DATA_CACHE_PREFIX)===0||key===SITE_DATA_CURRENT_VERSION_KEY||key===SITE_VERSION_CACHE_KEY||key==='xxy.cms.authError')remove.push(key);
  }
...
async function fetchSiteDataByVersion_(version){
 ...
 try{
  const dataUrl=SITE_DATA_FILE+'?v='+encodeURIComponent(normalizedVersion)+'&t='+Date.now();
  const data=await fetchJsonRequest_(dataUrl,'no-store');
