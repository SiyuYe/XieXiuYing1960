...
function heroCaptionLines(a){const first=[a.titleZh,a.titleEn,[a.year,a.size].filter(v=>String(v||'').trim()).join(' ')].filter(v=>String(v||'').trim()).join('｜');const second=valueList(a).join('｜');return [first,second].filter(Boolean);}
...
function initArtSections(){
 ...
 const image=document.querySelector('#heroImage');if(image){if(heroTimer){clearInterval(heroTimer);heroTimer=null;}image.loading='eager';image.decoding='async';image.setAttribute('fetchpriority','high');let slide=0;const t=document.querySelector('#heroTitle'),m=document.querySelector('#heroMeta');function draw(){const a=heroPool[slide];if(!a){image.removeAttribute('src');t.textContent='';m.textContent='';return;}image.src=imgSrc(a);const lines=heroCaptionLines(a);t.textContent=lines[0]||'';m.textContent=lines[1]||'';preloadHeroNeighbors_(heroPool,slide);}document.querySelector('.next')?.addEventListener('click',()=>{slide=(slide+1)%heroPool.length;draw()});document.querySelector('.prev')?.addEventListener('click',()=>{slide=(slide-1+heroPool.length)%heroPool.length;draw()});draw();if(heroPool.length>1)heroTimer=setInterval(()=>{slide=(slide+1)%heroPool.length;draw()},5000);}
 const featured=document.querySelector('#featuredWorks');if(featured)featured.innerHTML=feat.slice(0,8).map((a,i)=>card(a,i)).join('');
