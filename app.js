...
function renderCommonShell(){
 const brand=siteConfig?.brand||{};document.querySelectorAll('.brand').forEach(el=>el.innerHTML=`<span class="brand-mark"><img src="assets/icons/icon-192.png" alt="謝秀英代表作圖示"></span><span><strong>${esc(brand.zh||'謝秀英')}</strong><small>${esc(brand.en||'Xie Xiu-Ying')}</small></span>`);
...
function renderHome(){if(document.body.dataset.page!=='home'||!homeData)return;const h=homeData.hero||{},copy=document.querySelector('.hero-copy');if(copy)copy.innerHTML=`<div class="artist-profile-card"><img id="artistRandomPhoto" alt="謝秀英老師" hidden><div class="artist-profile-overlay"><p class="eyebrow">${esc(h.eyebrow||'')}</p><h1>${nl(h.title||'')}</h1><p class="hero-sub multiline">${nl(h.subtitle||'')}</p><div class="hero-actions"><a class="btn primary" href="${esc(h.primaryButton?.href||'gallery.html')}">${esc(h.primaryButton?.label||'進入線上藝廊')}</a><a class="btn ghost" href="${esc(h.secondaryButton?.href||siteConfig?.facebookUrl||'#')}" target="_blank" rel="noopener">${esc(h.secondaryButton?.label||'Facebook 粉專')}</a></div></div></div>`;loadRandomArtistPhoto();
 const newsSection=document.querySelector('#news');const showNotice=siteConfig?.showNotice===true;if(!showNotice){newsSection?.remove();document.querySelector('.home-quick a[href="#news"]')?.remove();}const news=document.querySelector('.news-grid');if(news&&Array.isArray(homeData.announcements))news.innerHTML=homeData.announcements.map(n=>`<article><time>${esc(n.date)}</time><h3>${esc(n.title)}</h3><p class="multiline">${nl(n.text)}</p></article>`).join('');
 ...
 renderRandomQuote();renderFacebook();
}
async function loadRandomArtistPhoto(){const img=document.querySelector('#artistRandomPhoto');if(!img)return;let urls=[];try{const r=await fetch('yingphoto/photos.json?v='+DATA_VERSION,{cache:'no-store'});if(r.ok){const data=await r.json();urls=(Array.isArray(data)?data:(data.photos||[])).map(x=>typeof x==='string'?x:(x.url||x.path||'')).filter(Boolean).map(x=>x.startsWith('http')?x:'yingphoto/'+x.replace(/^yingphoto\//,''));}}catch(e){}if(!urls.length){try{const owner=location.hostname.endsWith('.github.io')?location.hostname.split('.')[0]:'SiYuYeh';const r=await fetch(`https://api.github.com/repos/${owner}/XieXiuYing1960/contents/yingphoto`,{headers:{Accept:'application/vnd.github+json'}});if(r.ok){const data=await r.json();urls=(Array.isArray(data)?data:[]).filter(x=>x.type==='file'&&/\.(png|jpe?g|webp)$/i.test(x.name)).map(x=>x.download_url);}}catch(e){}}if(!urls.length)return;img.src=urls[Math.floor(Math.random()*urls.length)];img.hidden=false;}
