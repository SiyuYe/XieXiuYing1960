...
function heroCaptionLines(a) { var first = [a.titleZh, a.titleEn, [a.year, a.size].filter(function (v) { return String(v || '').trim(); }).join(' ')].filter(function (v) { return String(v || '').trim(); }).join('｜'); var second = valueList(a).join('｜'); return [first, second].filter(Boolean); }
...
function initArtSections() {
    ...
    if (image) {
        ...
        function draw() { var a = heroPool[slide_1]; if (!a) {
            ...
            m_1.textContent = '';
            return;
        } image.src = imgSrc(a); var lines = heroCaptionLines(a); t_1.textContent = lines[0] || ''; m_1.textContent = lines[1] || ''; preloadHeroNeighbors_(heroPool, slide_1); }
    ...
    if (featured)
        featured.innerHTML = feat.slice(0, 8).map(function (a, i) { return card(a, i); }).join('');
    ...
    if (works) {
        var drawWorks_1 = function (list) { return renderPagedArtGrid_(works, list); };
        drawWorks_1(artworks);
        document.querySelectorAll('.category-pills button').forEach(function (btn) { return btn.addEventListener('click', function () { document.querySelectorAll('.category-pills button').forEach(function (x) { return x.classList.remove('active'); }); btn.classList.add('active'); var key = btn.dataset.category || btn.textContent.trim(); drawWorks_1(key === '全部' ? artworks : artworks.filter(function (a) { return [a.subjectNames, a.artworkTypeName, a.categoryZh].some(function (v) { return String(v || '').includes(key); }); })); }); });
    }
    bindArtworkCards();
}
